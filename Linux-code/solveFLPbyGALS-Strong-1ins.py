...
def funGA_LS_DM_parallel():
    ...
    textFile = open(f'{fileName}_GASLSDM_EveInsData(m=2).txt', 'a')
    plotFile = open(f'{fileName}_GASLSDM_PlotData(m=2).txt', 'a')
    ...
    for i in range(iActualInsNum):
        ...
        plt.savefig(f'{fileName}_GASLSDM_Curve(m=2)-ins{str(i)}-oneRun.svg')
    ...
    textFile.write("\n-----------------------------------------------------\n")
    excelName = f'{fileName}_GASLSDM_ObjValueEveInsEveRun(m=2).xls'
...
def funGA_LS_DM():
    ...
    ax1.set_ylabel("Fitness Of Best Individual (× 1e-3)")
    ax2.set_ylabel("Diversity Metric")
    plt.savefig(f'{fileName}_GAALSDM_Curve(m=2).svg')
    # 将数据写入text文件
    textFile.write(
        f'Average CPU time of {str(iRunsNum)}' + ' runs for each instance:\n'
    )
