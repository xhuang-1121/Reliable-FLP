...
def funGA_LS_DM_single(fp_tuple_combOfInsRuns):
    ...
    # 调用GADM求解
    GeneticAlgo = GA_SLS_DM.GA(listGAParameters, fp_tuple_combOfInsRuns[0])
...
def funGA_LS_DM_parallel():
    ...
    textFile = open(fileName + '_GASLSDM_EveInsData(m=2).txt', 'a')
    plotFile = open(fileName + '_GASLSDM_PlotData(m=2).txt', 'a')
