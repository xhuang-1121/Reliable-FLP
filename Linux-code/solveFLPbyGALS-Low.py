...
def funGA_LS_DM_parallel():
    ...
    textFile = open(fileName + '_GALLSDM_EveInsData(m=2).txt', 'a')
    plotFile = open(fileName + '_GALLSDM_PlotData(m=2).txt', 'a')
    ...
    for i in range(iActualInsNum):
        ...
        plt.savefig(fileName + '_GALLSDM_Curve(m=2)-ins'+str(i)+'.svg')
    ...
    textFile.write("\n-----------------------------------------------------\n")
    excelName = fileName + '_GALLSDM_ObjValueEveInsEveRun(m=2).xls'
