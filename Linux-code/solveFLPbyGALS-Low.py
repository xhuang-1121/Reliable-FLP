...
def funGA_LS_DM_single(fp_tuple_combOfInsRuns):
    ...
    # 调用GADM求解
    GeneticAlgo = GA_LLS_DM.GA(listGAParameters, fp_tuple_combOfInsRuns[0], local_state)
...
def funGA_LS_DM_parallel():
    ...
    textFile = open(f'{fileName}_GALLSDM_EveInsData(m=2).txt', 'a')
    plotFile = open(f'{fileName}_GALLSDM_PlotData(m=2).txt', 'a')
    ...
    for i in range(iActualInsNum):
        ...
        plt.savefig(f'{fileName}_GALLSDM_Curve(m=2)-ins{str(i)}.svg')

    '''
    ax1.set_xlabel("# of Generation")
    ax1.set_ylabel("Fitness Of Best Individual (× 1e-3)")
    ax2.set_ylabel("Diversity Metric")
    plt.savefig(fileName + '_GALLSDM_Curve(m=2).svg')
    '''
    # 将数据写入text文件
    textFile.write(
    ...
    textFile.write("\n-----------------------------------------------------\n")
    excelName = f'{fileName}_GALLSDM_ObjValueEveInsEveRun(m=2).xls'
...
def funGA_LS_DM():
    ...
    for i in range(iActualInsNum):  # 8 instances
        ...
        for j in range(iRunsNum):  # Every instance has 10 runs experiments.
            ...
            cpuStart = time.process_time()
            # 调用GADM求解
            GeneticAlgo = GA_LLS_DM.GA(listGAParameters, ins)
    ...
    ax1.set_ylabel("Fitness Of Best Individual (× 1e-3)")
    ax2.set_ylabel("Diversity Metric")
    plt.savefig(f'{fileName}_GALSDM_Curve(m=2).svg')
    # 将数据写入text文件
    textFile.write(
        f'Average CPU time of {str(iRunsNum)}' + ' runs for each instance:\n'
    )
